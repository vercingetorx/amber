    ...
    fn rebuild_then_repair_tolerates_corrupted_parity_record() {
        ...
        crate::harden::append_cauchy_rs_parity(&archive, 200_000, None, None).unwrap();
    ...
    #[test]
    fn missing_anchors_preserves_cauchy_rs_metadata_after_rebuild() {
        let tmp = tempdir();
        let archive = build_sample_archive(&tmp, None);
        crate::harden::append_cauchy_rs_parity(&archive, 200_000, None, None).unwrap();
        ...
        let mut reader = ArchiveReader::new(&archive);
        reader.open().unwrap();
        assert!(!reader.cauchy_rs_parities.is_empty());
