    ...
    use super::append_cauchy_rs_parity;
    ...
    #[test]
    fn append_cauchy_rs_parity_rewrites_archive_with_more_parity() {
        let tmp = tempdir();
        ...
        before.open().unwrap();
        let before_parity = before.cauchy_rs_parities.len();
        assert!(before.verify().unwrap());
        drop(before);

        let added = append_cauchy_rs_parity(&archive, 30_000, None, None).unwrap();
        assert!(added > 0);
        ...
        assert!(after.cauchy_rs_parities.len() > before_parity);
    ...
    fn double_harden_preserves_mixed_row_count_repair() {
        ...
        let first_added = append_cauchy_rs_parity(&archive, 150_000, None, None).unwrap();
        let second_added = append_cauchy_rs_parity(&archive, 200_000, None, None).unwrap();
        ...
        let latest = groups
            ...
            .unwrap();
        let row_counts = get_map(latest, "cauchy_rs")
            .and_then(|cauchy_rs| get_list(cauchy_rs, "parity"))
            .unwrap()
            .iter()
    ...
    fn append_global_parity_enforces_canonical_min_total_parity_floor() {
        ...
        let latest = groups
            ...
            .max_by_key(|group| get_u64(group, "group_id").unwrap_or(0))
            .unwrap();
        let before_rows = get_map(latest, "cauchy_rs")
            .and_then(|cauchy_rs| get_list(cauchy_rs, "parity"))
            .map(|rows| rows.len())
            .unwrap_or(0);
        ...
        let added = append_cauchy_rs_parity(&archive, 0, None, None).unwrap();
        ...
        let latest = groups
            ...
            .max_by_key(|group| get_u64(group, "group_id").unwrap_or(0))
            .unwrap();
        let after_rows = get_map(latest, "cauchy_rs")
            .and_then(|cauchy_rs| get_list(cauchy_rs, "parity"))
            .map(|rows| rows.len())
            .unwrap_or(0);
    ...
    fn harden_failure_leaves_existing_archive_readable() {
        ...
        let baseline_rows = reader.cauchy_rs_parities.len();
        assert!(reader.verify().unwrap());
        drop(reader);
        ...
        let err = append_cauchy_rs_parity(&archive, 150_000, None, None).unwrap_err();
        ...
        assert_eq!(reader.cauchy_rs_parities.len(), baseline_rows);
