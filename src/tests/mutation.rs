    ...
    #[test]
    fn parity_archive_requires_explicit_cauchy_rs_scheme_for_rewrite() {
        ...
        cauchy_rs.insert("seed_base".into(), TlvValue::Bytes(vec![0u8; 16]));
        cauchy_rs.insert("parity".into(), TlvValue::List(Vec::new()));
        ...
        assert!(
            err.to_string()
                .contains("Cauchy RS metadata is missing its scheme")
        );
    }
