...
pub fn compute_cauchy_rs_payload(
    ...
    for (sym_index, coeff) in combo.iter().copied() {
        let data = symbol_bytes
            .get(&sym_index)
            .ok_or_else(|| format!("Cauchy RS source symbol {sym_index} is missing"))?;
        gf65536_mul_add_bytes(&mut parity, data, coeff);
    }
    Ok(parity)
}

pub fn iter_cauchy_rs_payloads<F>(
    start_row: usize,
    target: usize,
    combo_for_row: F,
    symbol_bytes: &BTreeMap<usize, Vec<u8>>,
    ...
    F: Fn(usize) -> Result<Vec<(usize, u16)>, String>,
{
    ...
    for row_id in start_row..(start_row + target) {
        ...
        let payload = compute_cauchy_rs_payload(symbol_bytes, &combo, symbol_size)?;
